...
import { PRIMARY_KEY_METADATA_KEY, REDIS_ENTITY_METADATA_KEY, RedisEventAlias, RedisEventChannel } from '../../core';
...
export class Repository<T extends Record<string, any>> {
  ...
  private startRedisInternalEventHandling(): void {
    ...
    if (this.onKeyExpired && this.repositoryOptions.keyspaceNotificationOptions?.expired) {
      this.redisNotifyKeyspaceEventsRegisterValue += RedisEventAlias.EXPIRED_EVENT;
      this.redisEventSubscriber.subscribe(RedisEventChannel.EXPIRED);
    }

    if (this.onKeyDeleted && this.repositoryOptions.keyspaceNotificationOptions?.deleted) {
      this.redisNotifyKeyspaceEventsRegisterValue += RedisEventAlias.GENERIC_COMMAND;
      this.redisEventSubscriber.subscribe(RedisEventChannel.DEL);
    }

    this.redisClient.config('SET', 'notify-keyspace-events', this.redisNotifyKeyspaceEventsRegisterValue);
  ...
  private setupRedisInternalEventHandler(): void {
    const handleEvent = async (channel: string, key: string) => {
      if (channel === RedisEventChannel.EXPIRED && this.doesKeyBelongToCurrentRepository(key)) {
        this.handleKeyExpiredEvent(key);
      }

      if (channel === RedisEventChannel.DEL && this.doesKeyBelongToCurrentRepository(key)) {
  ...
  private async handleKeyExpiredEvent(key: string): Promise<void> {
    ...
    const shadowCopy = plainToInstance(this.repositoryOptions.baseClass, JSON.parse(stringifiedValue!));
  ...
  protected onKeyExpired?(entity?: T | { primaryKey: string }): Promise<void> | void;
  protected onKeyDeleted?(primaryKey?: { primaryKey: string }): Promise<void> | void;
  ...
  public async delete(entity: Partial<T>): Promise<void> {
    ...
    if (this.repositoryOptions.keyspaceNotificationOptions?.expired?.passEntityToHandler) {
  ...
  public async get(entity: Partial<T>): Promise<T | null> {
    const key = entity[this.primaryKey];
    const stringifiedValue = await this.redisClient.get(`${this.storageKeyPrefix}:${key}`);
